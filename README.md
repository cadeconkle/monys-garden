...
A private book for the household Garden in Fuquay-Varina, North Carolina. One Gardener. Catalog, Garden, and a short curated Shop list.
...
4. For lock-screen Care events when the app is not open, set `VAPID_PUBLIC_KEY`, `VAPID_PRIVATE_KEY`, and `VAPID_SUBJECT` (a `mailto:` address). An hourly Function fetches the Growing-place forecast and updates those Care events so rain and frost still change the lock screen.
